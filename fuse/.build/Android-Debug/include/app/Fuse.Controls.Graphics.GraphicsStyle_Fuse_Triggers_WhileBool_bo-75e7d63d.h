// This file was generated based on '/usr/local/share/uno/Packages/Fuse.Controls/0.10.5/.cache/GeneratedCode/Graphics/$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_FUSE_CONTROLS_GRAPHICS_GRAPHICS_STYLE_FUSE_TRIGGERS_WHILE_BOOL_BO_75E7D63D_H__
#define __APP_FUSE_CONTROLS_GRAPHICS_GRAPHICS_STYLE_FUSE_TRIGGERS_WHILE_BOOL_BO_75E7D63D_H__

#include <app/Uno.UX.Property__bool.h>
#include <Uno.h>
namespace app { namespace Fuse { namespace Triggers { struct WhileBool; } } }

namespace app {
namespace Fuse {
namespace Controls {
namespace Graphics {

struct GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property;

struct GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property__uType : ::app::Uno::UX::Property__bool__uType
{
};

GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property__uType* GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property__typeof();

void GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property___ObjInit_1(GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property* __this, ::app::Fuse::Triggers::WhileBool* obj);
GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property* GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property__New_1(::uStatic* __this, ::app::Fuse::Triggers::WhileBool* obj);
bool GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property__OnGet(GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property* __this);
void GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property__OnSet(GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property* __this, bool v, ::uObject* origin);

struct GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property : ::app::Uno::UX::Property__bool
{
    ::uStrong< ::app::Fuse::Triggers::WhileBool*> _obj;

    void _ObjInit_1(::app::Fuse::Triggers::WhileBool* obj) { GraphicsStyle_Fuse_Triggers_WhileBool_bool_Value_Property___ObjInit_1(this, obj); }
};

}}}}


#endif
