// This file was generated based on '/usr/local/share/uno/Packages/UnoCore/0.12.0/Source/Uno/$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_UNO_FUNC__FUSE_CONTROLS_TEXT_CONTROL__FUSE_ELEMENTS_TEXT_ALIGNMENT_H__
#define __APP_UNO_FUNC__FUSE_CONTROLS_TEXT_CONTROL__FUSE_ELEMENTS_TEXT_ALIGNMENT_H__

#include <app/Uno.Delegate.h>
#include <Uno.h>

namespace app {
namespace Uno {

::uDelegateType* Func__Fuse_Controls_TextControl__Fuse_Elements_TextAlignment__typeof();

}}


#endif
