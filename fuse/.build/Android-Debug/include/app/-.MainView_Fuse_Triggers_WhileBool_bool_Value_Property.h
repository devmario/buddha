// This file was generated based on '/Users/jangwonhee/Documents/Buddha_2/fuse/.cache/GeneratedCode/MainView.g.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP___MAIN_VIEW_FUSE_TRIGGERS_WHILE_BOOL_BOOL_VALUE_PROPERTY_H__
#define __APP___MAIN_VIEW_FUSE_TRIGGERS_WHILE_BOOL_BOOL_VALUE_PROPERTY_H__

#include <app/Uno.UX.Property__bool.h>
#include <Uno.h>
namespace app { namespace Fuse { namespace Triggers { struct WhileBool; } } }

namespace app {

struct MainView_Fuse_Triggers_WhileBool_bool_Value_Property;

struct MainView_Fuse_Triggers_WhileBool_bool_Value_Property__uType : ::app::Uno::UX::Property__bool__uType
{
};

MainView_Fuse_Triggers_WhileBool_bool_Value_Property__uType* MainView_Fuse_Triggers_WhileBool_bool_Value_Property__typeof();

void MainView_Fuse_Triggers_WhileBool_bool_Value_Property___ObjInit_1(MainView_Fuse_Triggers_WhileBool_bool_Value_Property* __this, ::app::Fuse::Triggers::WhileBool* obj);
MainView_Fuse_Triggers_WhileBool_bool_Value_Property* MainView_Fuse_Triggers_WhileBool_bool_Value_Property__New_1(::uStatic* __this, ::app::Fuse::Triggers::WhileBool* obj);
bool MainView_Fuse_Triggers_WhileBool_bool_Value_Property__OnGet(MainView_Fuse_Triggers_WhileBool_bool_Value_Property* __this);
void MainView_Fuse_Triggers_WhileBool_bool_Value_Property__OnSet(MainView_Fuse_Triggers_WhileBool_bool_Value_Property* __this, bool v, ::uObject* origin);

struct MainView_Fuse_Triggers_WhileBool_bool_Value_Property : ::app::Uno::UX::Property__bool
{
    ::uStrong< ::app::Fuse::Triggers::WhileBool*> _obj;

    void _ObjInit_1(::app::Fuse::Triggers::WhileBool* obj) { MainView_Fuse_Triggers_WhileBool_bool_Value_Property___ObjInit_1(this, obj); }
};

}


#endif
