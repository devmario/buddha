// This file was generated based on '/usr/local/share/uno/Packages/UnoCore/0.12.0/Source/Uno/$.uno'.
// WARNING: Changes might be lost if you edit this file directly.

#ifndef __APP_UNO_ACTION__FUSE_CONTROLS_IMAGE__FUSE_RESOURCES_MEMORY_POLICY_H__
#define __APP_UNO_ACTION__FUSE_CONTROLS_IMAGE__FUSE_RESOURCES_MEMORY_POLICY_H__

#include <app/Uno.Delegate.h>
#include <Uno.h>

namespace app {
namespace Uno {

::uDelegateType* Action__Fuse_Controls_Image__Fuse_Resources_MemoryPolicy__typeof();

}}


#endif
